...
def get_external_passwd(directory, hostname):
...
def make_tmp_python_dir(directory):
...
if __name__ == "__main__":
    ...
    make_tmp_python_dir(options.directory)

    accounts = Accounts()
    hosts = read_hosts_file(options.filename)
  
    #create all the files to read 
    [ get_external_passwd(options.directory, host) for host in hosts ] 
    ...
    print("Hosts that were not completed:",  file=sys.stderr)
